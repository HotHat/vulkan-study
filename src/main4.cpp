...
int device_initialization(Init &init) {
    ...
    init.instance = builder.SetAppName("Example Vulkan Application")
    ...
    VkResult glfw_result = glfwCreateWindowSurface(init.instance.instance, init.window, nullptr, &init.surface);
    if (glfw_result != VK_SUCCESS) {
        std::cerr << "Failed to select create window surface. Error: " << std::to_string(glfw_result) << "\n";
        return EXIT_FAILURE;
    }

    lvk::PhysicalDeviceSelector phys_device_selector(init.instance);

    auto physical_device = phys_device_selector.SetSurface(init.surface).Select();
...
int create_swapchain(Init &init) {
    lvk::SwapchainBuilder swapchain_builder{init.device};
    auto swapchain = swapchain_builder.SetOldSwapchain(init.swapchain).Build();
...
int create_graphics_pipeline(Init &init, RenderData &data) {
    ...
    if (vkCreateGraphicsPipelines(init.device.device, VK_NULL_HANDLE, 1, &pipeline_info, nullptr, &data.graphics_pipeline) !=
        VK_SUCCESS) {
...
int create_framebuffers(Init &init, RenderData &data) {
    data.swapchain_images = init.swapchain.GetImages();
    data.swapchain_image_views = init.swapchain.GetImageViews();
...
int create_command_pool(Init &init, RenderData &data) {
    ...
    pool_info.queueFamilyIndex = init.device.GetQueueIndex(lvk::QueueType::kGraphics);
...
int create_sync_objects(Init &init, RenderData &data) {
    ...
    for (size_t i = 0; i < init.swapchain.image_count; i++) {
        if (vkCreateSemaphore(init.device.device, &semaphore_info, nullptr, &data.finished_semaphore[i]) != VK_SUCCESS) {
...
void cleanup(Init &init, RenderData &data) {
    ...
    lvk::destroy_surface(init.instance, init.surface);
    lvk::destroy_instance(init.instance);
