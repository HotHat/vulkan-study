...
SimpleDraw::SimpleDraw(VulkanContext &context_): context(context_) {
    create_render_pass();
    create_graphics_pipeline();
}

void SimpleDraw::create_render_pass() {
...
std::vector<char> SimpleDraw::readFile(const std::string &filename) {
...
VkShaderModule SimpleDraw::createShaderModule(const std::vector<char> &code) {
    ...
    if (vkCreateShaderModule(context.device.device, &create_info, nullptr, &shaderModule) != VK_SUCCESS) {
...
void SimpleDraw::create_graphics_pipeline() {
    ...
    if (vkCreatePipelineLayout(context.device.device, &pipeline_layout_info, nullptr, &pipeline_layout) != VK_SUCCESS) {
    ...
    if (vkCreateGraphicsPipelines(context.device.device, VK_NULL_HANDLE, 1, &pipeline_info, nullptr, &graphics_pipeline) !=
        VK_SUCCESS) {
