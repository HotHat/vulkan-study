...
PhysicalDevice PhysicalDeviceSelector::PopulateDeviceDetails(VkPhysicalDevice vk_phys_device, FeatureChain const& src_extended_features_chain) const {
    ...
    auto queue_families = get_vector_noerror<VkQueueFamilyProperties>( vkGetPhysicalDeviceQueueFamilyProperties, vk_phys_device);
    ...
    auto available_extensions_ret = get_vector<VkExtensionProperties>( available_extensions, vkEnumerateDeviceExtensionProperties, vk_phys_device, nullptr);
    ...
    if (!fill_chain.Empty() && (instance_is_1_1 || instance_info_.properties2_ext_enabled)) {
        ...
        if (instance_is_1_1) {
            ...
            //
            auto func =
                    get_instance_proc_addr<PFN_vkGetPhysicalDeviceFeatures2KHR>(instance_info_.instance, "vkGetPhysicalDeviceFeatures2KHR");
...
PhysicalDevice::Suitable PhysicalDeviceSelector::IsDeviceSuitable(
    ...
    bool dedicated_compute = get_dedicated_queue_index(pd.queue_families_, VK_QUEUE_COMPUTE_BIT, VK_QUEUE_TRANSFER_BIT) != QUEUE_INDEX_MAX_VALUE;
    bool dedicated_transfer = get_dedicated_queue_index(pd.queue_families_, VK_QUEUE_TRANSFER_BIT, VK_QUEUE_COMPUTE_BIT) != QUEUE_INDEX_MAX_VALUE;
    bool separate_compute = get_separate_queue_index(pd.queue_families_, VK_QUEUE_COMPUTE_BIT, VK_QUEUE_TRANSFER_BIT) != QUEUE_INDEX_MAX_VALUE;
    bool separate_transfer = get_separate_queue_index(pd.queue_families_, VK_QUEUE_TRANSFER_BIT, VK_QUEUE_COMPUTE_BIT) != QUEUE_INDEX_MAX_VALUE;

    bool present_queue = get_present_queue_index(pd.physical_device, instance_info_.surface, pd.queue_families_) != QUEUE_INDEX_MAX_VALUE;
    ...
    const auto unsupported_extensions = find_unsupported_extensions_in_list(pd.available_extensions_, criteria_.required_extensions);
    ...
    if (!criteria_.defer_surface_initialization && criteria_.require_present) {
        ...
        auto formats_ret = get_vector<VkSurfaceFormatKHR>(formats, vkGetPhysicalDeviceSurfaceFormatsKHR, pd.physical_device, instance_info_.surface);
        auto present_modes_ret = get_vector<VkPresentModeKHR>(present_modes, vkGetPhysicalDeviceSurfacePresentModesKHR,
                                                                      pd.physical_device,
        ...
        if (formats_ret != VK_SUCCESS || present_modes_ret != VK_SUCCESS || formats.empty() || present_modes.empty()) {
            ...
            if (formats.empty()) {
                unsuitability_reasons.emplace_back("vkGetPhysicalDeviceSurfaceFormatsKHR returned zero surface formats");
            }
...
std::vector<PhysicalDevice> PhysicalDeviceSelector::SelectDevices() const {
    ...
    auto vk_physical_devices_ret = get_vector<VkPhysicalDevice>(vk_physical_devices, vkEnumeratePhysicalDevices, instance_info_.instance);
    ...
    auto fill_out_phys_dev_with_criteria = [&](PhysicalDevice& phys_dev) {
        ...
        bool portability_ext_available =
                ...
                std::binary_search(phys_dev.available_extensions_.begin(), phys_dev.available_extensions_.end(), "VK_KHR_portability_subset");

        phys_dev.extensions_to_enable_.clear();
        phys_dev.extensions_to_enable_.insert(
                phys_dev.extensions_to_enable_.end(), criteria_.required_extensions.begin(), criteria_.required_extensions.end());
    ...
    if (criteria_.use_first_gpu_unconditionally && !vk_physical_devices.empty()) {
        PhysicalDevice physical_device = PopulateDeviceDetails(vk_physical_devices[0], criteria_.extended_features_chain);
...
bool PhysicalDevice::HasDedicatedComputeQueue() const {
    return get_dedicated_queue_index(queue_families_, VK_QUEUE_COMPUTE_BIT, VK_QUEUE_TRANSFER_BIT) != QUEUE_INDEX_MAX_VALUE;
}
bool PhysicalDevice::HasSeparateComputeQueue() const {
    return get_separate_queue_index(queue_families_, VK_QUEUE_COMPUTE_BIT, VK_QUEUE_TRANSFER_BIT) != QUEUE_INDEX_MAX_VALUE;
}
bool PhysicalDevice::HasDedicatedTransferQueue() const {
    return get_dedicated_queue_index(queue_families_, VK_QUEUE_TRANSFER_BIT, VK_QUEUE_COMPUTE_BIT) != QUEUE_INDEX_MAX_VALUE;
}
bool PhysicalDevice::HasSeparateTransferQueue() const {
    return get_separate_queue_index(queue_families_, VK_QUEUE_TRANSFER_BIT, VK_QUEUE_COMPUTE_BIT) != QUEUE_INDEX_MAX_VALUE;
}
...
bool PhysicalDevice::EnableExtensionIfPresent(const char* extension) {
    if (std::binary_search(std::begin(available_extensions_), std::end(available_extensions_), extension)) {
        extensions_to_enable_.insert(
                std::upper_bound(std::begin(extensions_to_enable_), std::end(extensions_to_enable_), extension), extension);
        return true;
    }
    return false;
}
bool PhysicalDevice::EnableExtensionsIfPresent(size_t count, const char* const* extensions) {
    ...
    for (size_t i = 0; i < count; ++i) {
        extensions_to_enable_.insert(
                std::upper_bound(std::begin(extensions_to_enable_), std::end(extensions_to_enable_), extensions[i]), extensions[i]);
    }
    return true;
}
